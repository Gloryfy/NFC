...
/** \file
* Wait Routines for Ncx3330 specific HAL-Component of Reader Library Framework.
*
* $Author$
* $Revision$ (v05.22.00)
* $Date$
...
phStatus_t phhalHw_Ncx3330_WaitIrq(
    ...
    )
{
    ...
    if ((bEnableIrq & PHHAL_HW_CHECK_IRQ_PIN_MASK) != PH_OFF)
    {
        ...
        if ((E_PH_OSAL_EVT_ABORT & tReceivedEvents) || (statusTmp != PH_ERR_SUCCESS))
        {
            ...
            /* Disable IRQ sources */
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegisterAndMask(pDataParams, IRQ_ENABLE, (uint32_t)~dwIrqWaitFor));
        ...
        else
        {
            ...
            *dwIrqReg = dwRegister;
            ...
            if ((bEnableIrq & PHHAL_HW_DISABLE_IRQ_CLEAR_MASK) == PH_OFF)
            {
                ...
                PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegister(pDataParams, IRQ_SET_CLEAR, dwRegister));
            }
            /* Disable IRQ sources */
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegisterAndMask(pDataParams, IRQ_ENABLE, (uint32_t)~dwIrqWaitFor));
