...
/** \file
* Internal functions for Ncx3330 specific HAL-Component of Reader Library Framework.
*
* $Author$
* $Revision$ (v05.22.00)
* $Date$
...
static const uint16_t PH_MEMLOC_CONST_ROM gkphhalHw_Ncx3330_DigiDelays_Us[][4] =
{
...
phStatus_t phhalHw_Ncx3330_GetTxBuffer(
    ...
    )
{
    ...
    {
        /* Redundant code */
        return phhalHw_Ncx3330_GetRxBuffer(pDataParams, bIsExchange, pTxBuffer, pTxBufferLen, pTxBufferSize);
    }

    return PH_ERR_SUCCESS;
}

phStatus_t phhalHw_Ncx3330_GetRxBuffer(
    phhalHw_Ncx3330_DataParams_t * pDataParams,
    uint8_t bIsExchange,
    uint8_t ** pRxBuffer,
...
phStatus_t phhalHw_Ncx3330_ReadData(
    phhalHw_Ncx3330_DataParams_t * pDataParams,
    uint8_t ** ppData,
    uint16_t * pBytesRead
    )
{
    ...
    /* Read the RX_STATUS */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_ReadRegister(
        pDataParams,
        RX_STATUS,
        &dwValue));
    ...
    if(wNumExpBytes != 0U)
    {
        /* Prepare the command for reading the RX FIFO of the Ncx3330 */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_RetrieveRxData(
            pDataParams,
            ppData,
            wNumExpBytes));
    }
...
phStatus_t phhalHw_Ncx3330_TimerStart(
    phhalHw_Ncx3330_DataParams_t * pDataParams,
    uint8_t bTimer,
    ...
    )
{
    ...
    wRegTypeValueSets[wSizeOfRegTypeValueSets++] = PHHAL_HW_NCx3330_WRITE_MULTIPLE_TYPE_WRITE;
...
phStatus_t phhalHw_Ncx3330_SetConfig_Int(
    phhalHw_Ncx3330_DataParams_t * pDataParams,
    uint16_t wConfig,
    uint16_t wValue
    )
{
    ...
    case PHHAL_HW_CONFIG_PARITY:
        ...
        if (wValue == PH_OFF)
        {
            ...
            /* Perform write */
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegisterAndMask(pDataParams, TX_CONFIG, dwValue));

            dwValue = (uint32_t)~(uint32_t)CRC_RX_CONFIG_RX_PARITY_ENABLE_MASK;
            /* Perform write */
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegisterAndMask(pDataParams, CRC_RX_CONFIG, dwValue));
        }
        else
        {
            /* Turn ON Tx-Parity */
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegisterOrMask(pDataParams, TX_CONFIG, TX_CONFIG_TX_PARITY_ENABLE_MASK));

            /* Turn ON Rx-Parity */
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegisterOrMask(pDataParams, CRC_RX_CONFIG, CRC_RX_CONFIG_RX_PARITY_ENABLE_MASK));
        }
    ...
    case PHHAL_HW_CONFIG_TXCRC:
        ...
        if (wValue == PH_OFF)
        {
            ...
            /* Perform write */
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegisterAndMask(pDataParams, CRC_TX_CONFIG, dwValue));
        }
        else
        {
    ...
    case PHHAL_HW_CONFIG_ASK100:
        ...
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegisterAndMask(pDataParams, ANA_RF_CONTROL_TX, ~(uint32_t)ANA_RF_CONTROL_TX_TX_RESIDUAL_CARRIER_MASK));
        }
...
phStatus_t phhalHw_Ncx3330_SetConfig_FelicaEmdReg(
    phhalHw_Ncx3330_DataParams_t * pDataParams
    )
{
    phStatus_t PH_MEMLOC_REM statusTmp;
    ...
    if (pDataParams->bOpeMode != RD_LIB_MODE_FELICA)
    {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegister(pDataParams, FELICA_EMD_CONTROL, (uint32_t)0x0U));
    ...
    else
    {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegister(pDataParams, FELICA_EMD_CONTROL, PHHAL_HW_NCx3330_DEFAULT_FELICA_EMD_REGISTER));
        /* Configure shadow register with default value */
        pDataParams->dwFelicaEmdReg = PHHAL_HW_NCx3330_DEFAULT_FELICA_EMD_REGISTER;
    }

    return PH_ERR_SUCCESS;
}

phStatus_t phhalHw_Ncx3330_SetConfig_FelicaEmdRegBit(
    phhalHw_Ncx3330_DataParams_t * pDataParams,
    uint16_t wValue,
    ...
    )
{
    ...
    if(wValue != PH_OFF)
    {
        if (!(pDataParams->dwFelicaEmdReg & dwMaskValue))
        {
            ...
            PH_CHECK_SUCCESS_FCT(statusTmp,
                phhalHw_Ncx3330_Instr_WriteRegisterOrMask(pDataParams, FELICA_EMD_CONTROL, dwMaskValue));
    ...
    else
    {
        if (pDataParams->dwFelicaEmdReg & dwMaskValue)
        {
            /* Update FELICA_EMD_CONTROL Register */
            PH_CHECK_SUCCESS_FCT(statusTmp,
                phhalHw_Ncx3330_Instr_WriteRegisterAndMask(pDataParams, FELICA_EMD_CONTROL, (uint32_t)~dwMaskValue));
            /* Update shadow register */
...
phStatus_t phhalHw_Ncx3330_SetConfig_FelicaEmdRegByte(
    phhalHw_Ncx3330_DataParams_t * pDataParams,
    uint16_t wValue,
    uint8_t bBytePos,
    uint32_t dwMaskValue
    )
{
    ...
    if ((uint8_t)(pDataParams->dwFelicaEmdReg >> bBytePos) != (uint8_t)wValue)
    {
        ...
        /* Update FELICA_EMD_CONTROL Register */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegister(pDataParams, FELICA_EMD_CONTROL, dwValue));
        /* Update shadow register */
...
phStatus_t phhalHw_Ncx3330_SetCardMode(
    phhalHw_Ncx3330_DataParams_t * pDataParams,
    ...
    uint16_t wSubcarrier )
{
    ...
    case PHHAL_HW_CARDTYPE_ISO14443A:
        ...
        if (pDataParams->wTargetMode != PH_OFF)
        {
            if(pDataParams->bActiveMode == PH_OFF)
            {
                switch (wTxDataRate)
                {
                    case PHHAL_HW_RF_DATARATE_106:
                        bTxConfig = PHHAL_HW_NCx3330_RF_TX_ISO14443A_PICC_106_MANCH_SUBC;
                        /*pTxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_106;*/
                        break;
                    case PHHAL_HW_RF_DATARATE_212:
                        bTxConfig = PHHAL_HW_NCx3330_RF_TX_ISO14443A_PICC_212_BPSK;
                        /*pTxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_212;*/
                        break;
                    case PHHAL_HW_RF_DATARATE_424:
                        bTxConfig = PHHAL_HW_NCx3330_RF_TX_ISO14443A_PICC_424_BPSK;
                        /* pTxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_424;*/
                        break;
                    case PHHAL_HW_RF_DATARATE_848:
                        bTxConfig = PHHAL_HW_NCx3330_RF_TX_ISO14443A_PICC_848_BPSK;
                        /* pTxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_848;*/
                        break;
                    case PHHAL_HW_RF_DATARATE_NO_CHANGE:
                        bTxConfig = PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX;
                        break;
                    default:
                        return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
                }
                ...
                {
                    case PHHAL_HW_RF_DATARATE_106:
                        bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO14443A_PICC_106_MILLER;
                        /*pRxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_106;*/
                        break;
                    case PHHAL_HW_RF_DATARATE_212:
                        bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO14443A_PICC_212_MILLER;
                        /*pRxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_212;*/
                        break;
                    case PHHAL_HW_RF_DATARATE_424:
                        bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO14443A_PICC_424_MILLER;
                        /* pRxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_424;*/
                        break;
                    case PHHAL_HW_RF_DATARATE_848:
                        bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO14443A_PICC_848_MILLER;
                        /* pRxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_848;*/
                        break;
                    case PHHAL_HW_RF_DATARATE_NO_CHANGE:
                        bRxConfig = PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX;
                        break;
                    default:
                        return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
                }
            ...
            else
            {
                ...
                {
                    case PHHAL_HW_RF_DATARATE_106:
                        bTxConfig = PHHAL_HW_NCx3330_RF_TX_NFC_AT_106;
                        break;
                    case PHHAL_HW_RF_DATARATE_212:
                        bTxConfig = PHHAL_HW_NCx3330_RF_TX_NFC_AT_212;
                        break;
                    case PHHAL_HW_RF_DATARATE_424:
                        bTxConfig = PHHAL_HW_NCx3330_RF_TX_NFC_AT_424;
                        break;
                    case PHHAL_HW_RF_DATARATE_NO_CHANGE:
                        bTxConfig = PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX;
                        break;
                ...
                {
                    case PHHAL_HW_RF_DATARATE_106:
                        bRxConfig = PHHAL_HW_NCx3330_RF_RX_NFC_AT_106;
                        break;
                    case PHHAL_HW_RF_DATARATE_212:
                        bRxConfig = PHHAL_HW_NCx3330_RF_RX_NFC_AT_212;
                        break;
                    case PHHAL_HW_RF_DATARATE_424:
                        bRxConfig = PHHAL_HW_NCx3330_RF_RX_NFC_AT_424;
                        break;
                    case PHHAL_HW_RF_DATARATE_NO_CHANGE:
                        bRxConfig = PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX;
                        break;
        ...
        else
        {
            ...
            {
                case PHHAL_HW_RF_DATARATE_106:
                    bTxConfig = PHHAL_HW_NCx3330_RF_TX_ISO14443A_106_MILLER;
                    /*pTxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_106;*/
                    break;
                case PHHAL_HW_RF_DATARATE_212:
                    bTxConfig = PHHAL_HW_NCx3330_RF_TX_ISO14443A_212_MILLER;
                    /*pTxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_212;*/
                    break;
                case PHHAL_HW_RF_DATARATE_424:
                    bTxConfig = PHHAL_HW_NCx3330_RF_TX_ISO14443A_424_MILLER;
                    /* pTxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_424;*/
                    break;
                case PHHAL_HW_RF_DATARATE_848:
                    bTxConfig = PHHAL_HW_NCx3330_RF_TX_ISO14443A_848_MILLER;
                    /* pTxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_848;*/
                    break;
                case PHHAL_HW_RF_DATARATE_NO_CHANGE:
                    bTxConfig = PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX;
            ...
            {
                case PHHAL_HW_RF_DATARATE_106:
                    bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO14443A_106_MANCH_SUBC;
                    /*pRxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_106;*/
                    break;
                case PHHAL_HW_RF_DATARATE_212:
                    bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO14443A_212_BPSK;
                    /*pRxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_212;*/
                    break;
                case PHHAL_HW_RF_DATARATE_424:
                    bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO14443A_424_BPSK;
                    /* pRxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_424;*/
                    break;
                case PHHAL_HW_RF_DATARATE_848:
                    bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO14443A_848_BPSK;
                    /* pRxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443A_848;*/
                    break;
                case PHHAL_HW_RF_DATARATE_NO_CHANGE:
                    bRxConfig = PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX;
    ...
    case PHHAL_HW_CARDTYPE_ISO14443B:
        ...
        {
            case PHHAL_HW_RF_DATARATE_106:
                bTxConfig = PHHAL_HW_NCx3330_RF_TX_ISO14443B_106_NRZ;
                /* pTxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443B_106;*/
                break;
            case PHHAL_HW_RF_DATARATE_212:
                bTxConfig = PHHAL_HW_NCx3330_RF_TX_ISO14443B_212_NRZ;
                /* pTxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443B_212;*/
                break;
            case PHHAL_HW_RF_DATARATE_424:
                bTxConfig = PHHAL_HW_NCx3330_RF_TX_ISO14443B_424_NRZ;
                /*pTxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443B_424;*/
                break;
            case PHHAL_HW_RF_DATARATE_848:
                bTxConfig = PHHAL_HW_NCx3330_RF_TX_ISO14443B_848_NRZ;
                /*pTxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443B_848;*/
                break;
            case PHHAL_HW_RF_DATARATE_NO_CHANGE:
                bTxConfig = PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX;
                break;
        ...
        {
            case PHHAL_HW_RF_DATARATE_106:
                bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO14443B_106_BPSK;
                /* pRxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443B_106;*/
                break;
            case PHHAL_HW_RF_DATARATE_212:
                bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO14443B_212_BPSK;
                /*pRxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443B_212;*/
                break;
            case PHHAL_HW_RF_DATARATE_424:
                bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO14443B_424_BPSK;
                /* pRxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443B_424;*/
                break;
            case PHHAL_HW_RF_DATARATE_848:
                bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO14443B_848_BPSK;
                /*  pRxRegisterSet = (const uint8_t*)gkphhalHw_Ncx3330_I14443B_848;*/
                break;
            case PHHAL_HW_RF_DATARATE_NO_CHANGE:
                bRxConfig = PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX;
                break;
    ...
    case PHHAL_HW_CARDTYPE_FELICA_424:
        ...
        if (pDataParams->wTargetMode != PH_OFF)
        {
            if(pDataParams->bActiveMode == PH_OFF)
            {
                ...
                {
                    case PHHAL_HW_RF_DATARATE_212:
                        bTxConfig = PHHAL_HW_NCx3330_RF_TX_NFC_PT_212;
                        break;

                    case PHHAL_HW_RF_DATARATE_424:
                        bTxConfig = PHHAL_HW_NCx3330_RF_TX_NFC_PT_424;
                        break;
                    case PHHAL_HW_RF_DATARATE_NO_CHANGE:
                        bTxConfig = PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX;
                        break;
                ...
                {
                    case PHHAL_HW_RF_DATARATE_212:
                        bRxConfig = PHHAL_HW_NCx3330_RF_RX_NFC_PT_212;
                        break;

                    case PHHAL_HW_RF_DATARATE_424:
                        bRxConfig = PHHAL_HW_NCx3330_RF_RX_NFC_PT_424;
                        break;
                    case PHHAL_HW_RF_DATARATE_NO_CHANGE:
                        bRxConfig = PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX;
                        break;
        ...
        else
        {
            ...
            {
                case PHHAL_HW_RF_DATARATE_212:
                    bTxConfig = PHHAL_HW_NCx3330_RF_TX_FELICA_212;
                    break;

                case PHHAL_HW_RF_DATARATE_424:
                    bTxConfig = PHHAL_HW_NCx3330_RF_TX_FELICA_424;
                    break;
            ...
            {
                case PHHAL_HW_RF_DATARATE_212:
                    bRxConfig = PHHAL_HW_NCx3330_RF_RX_FELICA_212;
                    break;

                case PHHAL_HW_RF_DATARATE_424:
                    bRxConfig = PHHAL_HW_NCx3330_RF_RX_FELICA_424;
                    break;
    ...
    case PHHAL_HW_CARDTYPE_ISO15693:
        ...
        switch (wTxDataRate)
        {
            case PHHAL_HW_RF_TX_DATARATE_1_OUT_OF_4:
                bTxConfig = PHHAL_HW_NCx3330_RF_TX_ISO15693_26_1OF4_ASK10;
                break;
        ...
        switch (wRxDataRate)
        {
            case PHHAL_HW_RF_RX_DATARATE_HIGH:
                bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO15693_26_1OF4_SC;
                break;

            case PHHAL_HW_RF_RX_DATARATE_FAST_HIGH:
                bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO15693_53_1OF4_SC;
                break;
    ...
    case PHHAL_HW_CARDTYPE_I18000P3M3:
        ...
        switch (wTxDataRate)
        {
            case PHHAL_HW_RF_TX_DATARATE_I18000P3M3:
                ...
                bTxConfig = PHHAL_HW_NCx3330_RF_TX_ISO180003M3_TARI_9_44_ASK;
                break;
        ...
        switch (wRxDataRate)
        {
            case PHHAL_HW_RX_I18000P3M3_FL_423_MAN2:
                bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO180003M3_MANCH424_2_PERIOD;
                break;
            case PHHAL_HW_RX_I18000P3M3_FL_423_MAN4:
                bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO180003M3_MANCH424_4_PERIOD;
                break;
            case PHHAL_HW_RX_I18000P3M3_FL_847_MAN2:
                bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO180003M3_MANCH848_2_PERIOD;
                break;
            case PHHAL_HW_RX_I18000P3M3_FL_847_MAN4:
                bRxConfig = PHHAL_HW_NCx3330_RF_RX_ISO180003M3_MANCH848_4_PERIOD;
                break;
            case PHHAL_HW_RF_DATARATE_NO_CHANGE:
                bRxConfig = PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX;
                break;
    ...
    case PHHAL_HW_CARDTYPE_I18092M_ACTIVE_424:
        ...
        {
            case PHHAL_HW_RF_DATARATE_106:
                bTxConfig = PHHAL_HW_NCx3330_RF_TX_NFC_AI_106_106;
                break;

            case PHHAL_HW_RF_DATARATE_212:
                bTxConfig = PHHAL_HW_NCx3330_RF_TX_NFC_AI_212_212;
            break;

            case PHHAL_HW_RF_DATARATE_424:
                bTxConfig = PHHAL_HW_NCx3330_RF_TX_NFC_AI_424_424;
                break;
        ...
        {
            case PHHAL_HW_RF_DATARATE_106:
                bRxConfig = PHHAL_HW_NCx3330_RF_RX_NFC_AI_106;
                break;

            case PHHAL_HW_RF_DATARATE_212:
                bRxConfig = PHHAL_HW_NCx3330_RF_RX_NFC_AI_212;
            break;

            case PHHAL_HW_RF_DATARATE_424:
                bRxConfig = PHHAL_HW_NCx3330_RF_RX_NFC_AI_424;
                break;
    ...
    if((PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX != bTxConfig) || (PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX != bRxConfig))
    {
        PH_CHECK_SUCCESS_FCT(statusTmp,
    ...
    if(wTxDataRate != PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX)
    {
        pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXDATARATE_FRAMING] = wTxDataRate;
    }
    if(wRxDataRate != PHHAL_HW_NCx3330_CURRENT_RF_CONFIGURATION_INDEX)
    {
...
phStatus_t phhalHw_Ncx3330_SetTmo(
    phhalHw_Ncx3330_DataParams_t *pDataParams,
    uint16_t wTimeout,
    uint8_t  bUnit
    )
{
    ...
    if (bUnit == PHHAL_HW_TIME_MICROSECONDS)
    {
        ...
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Int_GetDigiDelay(
                pDataParams,
                PH_ON,
                &wUDigitalDelay));
        }
        ...
        dwLoadValue =(uint32_t) ( PHHAL_HW_NCx3330_MAX_FREQ / (PHHAL_HW_NCx3330_CONVERSION_US_SEC/100));
    ...
    {
        wPrescaler = 0x3CU;
        if(wTimeout <= PHHAL_HW_NCx3330_MAX_TIME_DELAY_MS)
        {
            dwLoadValue =(uint32_t) (  wTimeout * ( PHHAL_HW_NCx3330_MIN_FREQ  / PHHAL_HW_NCx3330_CONVERSION_MS_SEC) );
        }
        else
        {
...
phStatus_t phhalHw_Ncx3330_Int_Wait(
    phhalHw_Ncx3330_DataParams_t * pDataParams,
    uint32_t dwLoadValue,
    uint32_t wPrescaler)
{
    ...
    /* Clear Interrupt  */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegister(pDataParams, IRQ_SET_CLEAR, IRQ_SET_CLEAR_TIMER0_IRQ_CLR_MASK));

    phOsal_EventClear(&pDataParams->HwEventObj.EventHandle, E_OS_EVENT_OPT_NONE, E_PH_OSAL_EVT_RF, NULL);

    /* Enable IRQ source */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegister(pDataParams, IRQ_ENABLE, IRQ_ENABLE_TIMER0_IRQ_SET_ENABLE_MASK));

    /*Start the timer*/
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegisterMultiple( pDataParams, wRegTypeValueSets, wSizeOfRegTypeValueSets));

    /* Wait for the timer0 to expire. */
    PH_CHECK_SUCCESS_FCT(statusTmp,phhalHw_Ncx3330_WaitIrq(pDataParams,PH_ON,PH_OFF,IRQ_STATUS_TIMER0_IRQ_MASK,&dwIrqStatusReg));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegisterAndMask(pDataParams,TIMER0_CONFIG, (uint32_t)(~TIMER0_CONFIG_T0_ENABLE_MASK)));

    return PH_ERR_SUCCESS;
}

phStatus_t phhalHw_Ncx3330_Int_GetDigiDelay(
                                      phhalHw_Ncx3330_DataParams_t * pDataParams,
                                      uint8_t bIsTimeout,
                                      uint16_t * pDelayUs
                                      )
{
    ...
    case PHHAL_HW_CARDTYPE_ISO14443A:
    case PHHAL_HW_CARDTYPE_ISO14443B:
        *pDelayUs = gkphhalHw_Ncx3330_DigiDelays_Us[bIndex][wDataRate];
        /* In a timeout case we have to add an additional delay */
        if (bIsTimeout != PH_OFF)
        {
            *pDelayUs = *pDelayUs + PHHAL_HW_NCx3330_I14443_ADD_DELAY_US;
        }
    ...
    case PHHAL_HW_CARDTYPE_FELICA_424:
        ...
        *pDelayUs = gkphhalHw_Ncx3330_DigiDelays_Us[bIndex][wDataRate];
        break;
    case PHHAL_HW_CARDTYPE_ISO15693:
        *pDelayUs = gkphhalHw_Ncx3330_DigiDelays_Us[bIndex][wDataRate - PHHAL_HW_RF_RX_DATARATE_LOW];
        break;
    case PHHAL_HW_CARDTYPE_ICODEEPCUID:
        *pDelayUs = gkphhalHw_Ncx3330_DigiDelays_Us[bIndex][0];
        break;
    case PHHAL_HW_CARDTYPE_I18000P3M3:
        switch (wDataRate)
        {
            case PHHAL_HW_RX_I18000P3M3_FL_423_MAN2:
                *pDelayUs = gkphhalHw_Ncx3330_DigiDelays_Us[bIndex][0];
                break;
            case PHHAL_HW_RX_I18000P3M3_FL_423_MAN4:
                *pDelayUs = gkphhalHw_Ncx3330_DigiDelays_Us[bIndex][1];
                break;
            case PHHAL_HW_RX_I18000P3M3_FL_847_MAN2:
                *pDelayUs = gkphhalHw_Ncx3330_DigiDelays_Us[bIndex][2];
                break;
            case PHHAL_HW_RX_I18000P3M3_FL_847_MAN4:
                *pDelayUs = gkphhalHw_Ncx3330_DigiDelays_Us[bIndex][3];
                break;
            default:
                return PH_ADD_COMPCODE_FIXED(PH_ERR_INTERNAL_ERROR, PH_COMP_HAL);
        }
...
phStatus_t phhalHw_Ncx3330_Int_GetFdt(
                                phhalHw_Ncx3330_DataParams_t * pDataParams,
                                ...
                                )
{
    ...
    /* Get T1 reload value */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_ReadRegister(pDataParams, TIMER1_RELOAD, &bRegister));
    ...
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_ReadRegister(pDataParams, TIMER1_STATUS, &bRegister));
    ...
    {
        FREQ = PHHAL_HW_NCx3330_MIN_FREQ/1000;
        wT1Value *= PHHAL_HW_NCx3330_CONVERSION_MS_SEC;
    }
    else
    {
        FREQ = PHHAL_HW_NCx3330_MAX_FREQ/1000000;
        wT1Value *= PHHAL_HW_NCx3330_CONVERSION_US_SEC;
    }
    ...
    *pTime = (uint32_t)fTime;
    ...
    if (pDataParams->wFirmwareVer < 0x308U)
    {
        ...
        if ((wExchangeStatus & PH_ERR_MASK) != PH_ERR_IO_TIMEOUT)
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Int_GetDigiDelay(pDataParams, PH_OFF, &wDelay));
            *pTime = *pTime - wDelay;
        }
    }
...
phStatus_t phhalHw_Ncx3330_Receive_Int(phhalHw_Ncx3330_DataParams_t * pDataParams,
                                      ...
                                      )
{
   ...
   else if ((dwIrqReg & IRQ_STATUS_TIMER1_IRQ_MASK))
   {
        ...
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_FieldReset(pDataParams));
        }
    ...
    else if ((pDataParams->bActiveMode == PH_ON) && (0U != ((dwIrqReg & IRQ_STATUS_RF_ACTIVE_ERROR_IRQ_MASK))))
    {
        ...
        if (pDataParams->wTargetMode != PH_OFF)
        {
            ...
            {
                status = PHHAL_HW_NCx3330_ACTIVE_ERR_CAUSE4;
            }
        }
        else
        {
    ...
    if ((0U != ((dwIrqReg & IRQ_STATUS_RX_IRQ_MASK))) && (status == PH_ERR_SUCCESS))
    {
...
#endif
        /* Read the RX_STATUS */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_ReadRegister(pDataParams, RX_STATUS, &dwValue));
        ...
        if (wTmpBufferLen > PHHAL_HW_NCx3330_MAX_FSD)  // Frame Size > FSD
        {
            status = PH_ERR_BUFFER_OVERFLOW;
        }
    }
...
phStatus_t phhalHw_Ncx3330_Int_IdleCommand(phhalHw_Ncx3330_DataParams_t * pDataParams)
{
    phStatus_t  PH_MEMLOC_REM statusTmp;
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegisterAndMask(
        pDataParams,
        SYSTEM_CONFIG,
        (uint32_t)~(SYSTEM_CONFIG_COMMAND_MASK)));
#ifndef _WIN32
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_WriteRegister(pDataParams, IRQ_SET_CLEAR, IRQ_SET_CLEAR_IDLE_IRQ_CLR_MASK));
#endif

    return PH_ERR_SUCCESS;
}

phStatus_t phhalHw_Ncx3330_Int_LoadCommand(phhalHw_Ncx3330_DataParams_t * pDataParams, uint8_t bCmd)
{
    phStatus_t  PH_MEMLOC_REM statusTmp;
...
phStatus_t phhalHw_Ncx3330_AutoSyncByte(phhalHw_Ncx3330_DataParams_t * pDataParams)
{
    phStatus_t PH_MEMLOC_REM statusTmp;
    uint32_t PH_MEMLOC_REM dwValue;
    uint32_t PH_MEMLOC_REM dwTemp;
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3330_Instr_ReadRegister(pDataParams, SIGPRO_CONFIG, &dwValue));
