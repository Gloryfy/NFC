...
phStatus_t phDriver_TimerStart(phDriver_Timer_Unit_t eTimerUnit, uint32_t dwTimePeriod, pphDriver_TimerCallBck_t pTimerCallBack)
{
    ...
    /* lpit1_ChnConfig0 initialized by Processor Expert */
    /* Initialize LPIT instance 0
     *  -   Reset and enable peripheral
     */
    LPIT_DRV_Init(PH_DRIVER_S32K_LPIT_INSTANCE, &lpit1_InitConfig);
    ...
    INT_SYS_InstallHandler(LPIT_Channel_IRQn, &LPIT0_Ch0_IRQHandler, (isr_t *)0);
...
phStatus_t phDriver_TimerStop(void)
{
    ...
    /* Disable interrupt for LPIT0 channel 0 - here we use Deinit */
    LPIT_DRV_Deinit(PH_DRIVER_S32K_LPIT_INSTANCE);
...
{
#if 0 /* currently all pin configuration is done in processor expert */
    uint8_t bPinNum;
...
void LPIT0_Ch0_IRQHandler(void)
{
	if (LPIT_DRV_GetInterruptFlagTimerChannels(PH_DRIVER_S32K_LPIT_INSTANCE,(1 << PH_DRIVER_S32K_LPIT_CHANNEL)))
	{
