...
int main (void)
{
    ...
    do
    {
        ...
        //uint8_t can_test_data[9]={0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08};
        ...
        NFC_CAN_Config();// kai 2020.01.07
        //for(uint32_t j=0;j<50;j++)
        //{
        //	NFC_CAN_Send_Frame(0x2AA,can_test_data,2);// kai 2020.01.07
        //}
...
void BasicDiscoveryLoop_Demo(void  *pDataParams)
{
    ...
    while(1)
    {
        ...
        //DEBUG_PRINTF("\t\t [User Log]status1 = 0x%04x wEntryPoint = %d\n\r",(status & PH_ERR_MASK),wEntryPoint);
        status = phacDiscLoop_Run(pDataParams, wEntryPoint);
        //DEBUG_PRINTF("\t\t [User Log]status2 = 0x%04x wEntryPoint = %d\n\r",(status & PH_ERR_MASK),wEntryPoint);
