                                                  ...
                                                  )
{
	...
	uint16_t   PH_MEMLOC_REM   wEmdMode = 0x0000;
    ...
    for (bTechIndex = 0; bTechIndex < PHAC_DISCLOOP_PASS_POLL_MAX_TECHS_SUPPORTED; bTechIndex++)
    {
        ...
        if (0U != (bDetectConfig & (PH_ON << bTechIndex)))
        {
			...
			if (status == PH_ERR_SUCCESS && wEmdMode == PH_ON)
			{
	            /* Detect disturber and change register settings */
				PH_CHECK_SUCCESS_FCT(status, phhalHw_DetectEMD(pDataParams->pHalDataParams));
			}
