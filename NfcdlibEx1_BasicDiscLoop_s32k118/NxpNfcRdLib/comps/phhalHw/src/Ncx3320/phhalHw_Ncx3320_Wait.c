                                 ...
                                 )
{
    ...
    if (bEnableIrqs != PH_OFF)
    {
        ...
        bIrq0EnReg = bRegister & PHHAL_HW_NCx3320_BIT_IRQINV;
    ...
    do
    {
        ...
        if (bWaitUntilPowerUp != PH_OFF)
        {
            ...
            status = phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_COMMAND, &bRegister);
            ...
            if ((status & PH_ERR_MASK) != PH_ERR_IO_TIMEOUT)
            {
                ...
                if (0U == (bRegister & PHHAL_HW_NCx3320_BIT_STANDBY))
                {
                    status = phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ1, &bRegister);
    ...
    while (0U == (bRegister & PHHAL_HW_NCx3320_BIT_GLOBALIRQ));
    ...
    if (bWaitUntilPowerUp != PH_OFF)
    {
        do
        {
            status = phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_COMMAND, PHHAL_HW_NCx3320_CMD_IDLE);
        }
        while ((status & PH_ERR_MASK) == PH_ERR_IO_TIMEOUT);
    }
#endif /* _WIN32 */
#ifndef _WIN32
    //DEBUG_PRINTF("[User Log] before phOsal_EventPend\r\n");
    ...
    //DEBUG_PRINTF("[User Log] after phOsal_EventPend\r\n");
    ...
    if (NULL != pIrq1Reg)
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ1, pIrq1Reg));
    }
    /* return Irq0 status */
    if (NULL != pIrq0Reg)
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ0, pIrq0Reg));
    }
