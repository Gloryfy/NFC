...
#define SET_NCx3320_SHADOW(USED_SHADOW)   \
...
static const uint16_t PH_MEMLOC_CONST_ROM wNcx3320_Emvco_I14443a_DefaultShadow[][2] =
{
    {PHHAL_HW_CONFIG_PARITY,                PH_ON},
    {PHHAL_HW_CONFIG_TXCRC,                 PH_OFF},
    {PHHAL_HW_CONFIG_RXCRC,                 PH_OFF},
...
static const uint16_t PH_MEMLOC_CONST_ROM wNcx3320_Emvco_I14443b_DefaultShadow[][2] =
{
    {PHHAL_HW_CONFIG_PARITY,                PH_OFF},
    {PHHAL_HW_CONFIG_TXCRC,                 PH_ON},
    {PHHAL_HW_CONFIG_RXCRC,                 PH_ON},
...
static const uint16_t PH_MEMLOC_CONST_ROM wNcx3320_NFC_I14443b_DefaultShadow[][2] =
{
    {PHHAL_HW_CONFIG_PARITY,                PH_OFF},
    {PHHAL_HW_CONFIG_TXCRC,                 PH_ON},
    {PHHAL_HW_CONFIG_RXCRC,                 PH_ON},
...
static const uint16_t PH_MEMLOC_CONST_ROM wNcx3320_DefaultShadow_Felica_424[][2] =
{
    {PHHAL_HW_CONFIG_PARITY,                PH_OFF},
    {PHHAL_HW_CONFIG_TXCRC,                 PH_ON},
    {PHHAL_HW_CONFIG_RXCRC,                 PH_ON},
...
static const uint16_t PH_MEMLOC_CONST_ROM wNcx3320_DefaultShadow_NfcDep[][2] =
{
    {PHHAL_HW_CONFIG_PARITY,                PH_ON},
    {PHHAL_HW_CONFIG_TXCRC,                 PH_ON},
    {PHHAL_HW_CONFIG_RXCRC,                 PH_ON},
...
static void phhalHw_Ncx3320_Reset(void);
static void phhalHw_Ncx3320_EventCallback(void * pDataParams);
static phStatus_t phhalHw_Ncx3320_ConfigTimeout(phhalHw_Ncx3320_DataParams_t * pDataParams, uint16_t wConfig, uint16_t wValue);
static phStatus_t phhalHw_Ncx3320_TransmitJewelCmd(
...
{
#ifndef _WIN32
    phhalHw_Ncx3320_DataParams_t * pNcx3320DataParams = NULL;

    pNcx3320DataParams = (phhalHw_Ncx3320_DataParams_t*) pDataParams;
    /* Post Abort Event. */
    (void)phOsal_EventPost(&pNcx3320DataParams->HwEventObj.EventHandle, E_OS_EVENT_OPT_POST_ISR, E_PH_OSAL_EVT_RF, NULL);
#else /* _WIN32 */
    /* satisfy compiler */
    if (0U != (pDataParams));
#endif /* _WIN32 */
...
static phStatus_t phhalHw_Ncx3320_ConfigTimeout(phhalHw_Ncx3320_DataParams_t * pDataParams, uint16_t wConfig, uint16_t wValue)
{
    ...
    if (pDataParams->bPollGuardTimeFlag == PH_OFF)
    {
        ...
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_T0CONTROL, &bRegister));
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T0CONTROL, bRegister | PHHAL_HW_NCx3320_BIT_TAUTORESTARTED | PHHAL_HW_NCx3320_VALUE_TCLK_1356_MHZ));

            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_T1CONTROL, &bRegister));
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T1CONTROL, bRegister | PHHAL_HW_NCx3320_VALUE_TCLK_T0));
        }
                              ...
                              )
{
    ...
    pDataParams->bFifoSize              = PH_NXPNFCRDLIB_CONFIG_NCx3320_DEFAULT_FIFOSIZE;
    pDataParams->wMaxPrecachedBytes     = PH_NXPNFCRDLIB_CONFIG_NCx3320_DEFAULT_PRECACHED_BYTES;
    pDataParams->wAdditionalInfo        = 0;
    pDataParams->bRfResetAfterTo        = PH_OFF;
    pDataParams->bMode                  = PHHAL_HW_NCx3320_CMD_LPCD_MODE_DEFAULT;
	...
	pDataParams->bEmdMode               = PH_OFF;
	pDataParams->bEmdTimeout            = 10;
	pDataParams->bEmdThreshold          = 0xFC;
	pDataParams->bEmdReceiveGain        = 0x01;
    pDataParams->bPollGuardTimeFlag     = PH_OFF;
    pDataParams->wCurrentSlotCount      = 0;
    pDataParams->bSession               = 0;
    pDataParams->b18000p3m3Q            = 0;
    pDataParams->pRFISRCallback         = &phhalHw_Ncx3320_EventCallback;
    /* Disable Charge Pump and Filter By Default. */
    pDataParams->bLpcdOption            = 0;
	pDataParams->bTxiDelta              = 1U;
	pDataParams->bTxiChannel            = 0U;
	pDataParams->bTxiDirection          = 0U;

    /* Reset sub-carrier setting, needed for phhalHw_Ncx3320_SetCardMode */
     ...
     * */
    PH_CHECK_SUCCESS_FCT(status, phhalHw_Ncx3320_WriteRegister(
        pDataParams,
        PHHAL_HW_NCx3320_REG_IRQ0EN,
        bIrqType
        ));
                                  ...
                                  )
{
    ...
    do
    {
            ...
            (0U == (wOption & PH_EXCHANGE_LEAVE_BUFFER_BIT)))
        {
                ...
                (pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO14443A))
            {
                ...
                PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_GetTxBuffer(pDataParams, PH_ON, &pTmpBuffer, &wTmpBufferLen, &wTmpBufferSize));
        ...
        if(pDataParams->bJewelActivated == PH_ON)
        {
            ...
            PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_SetConfig(pDataParams, PHHAL_HW_CONFIG_PARITY, PH_OFF));

            /* Make sure CRC is OFF */
            PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_SetConfig(pDataParams, PHHAL_HW_CONFIG_TXCRC, PH_OFF));
            PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_SetConfig(pDataParams, PHHAL_HW_CONFIG_RXCRC, PH_OFF));

            /* First Jewel byte to be transmitted is with 7 bits. */
            PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_SetConfig(pDataParams, PHHAL_HW_CONFIG_TXLASTBITS, 7));
        }
        ...
        if(0U != (pDataParams->bCRC5Enabled))
        {
             ...
             */
            PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_GetConfig(pDataParams, PHHAL_HW_CONFIG_TXCRC, &wIndex));
            PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_GetConfig(pDataParams, PHHAL_HW_CONFIG_RXCRC, &wVerifyRxCrc5));
            ...
            if(0U != (wIndex))
            {
                ...
                PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_GetConfig(pDataParams, PHHAL_HW_CONFIG_TXLASTBITS, &wIndex));
    ...
    if (statusTmp == PH_ERR_SUCCESS)
    {
        ...
        do
        {
                ...
                (wTxLength == 0U))
            {
                ...
                PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_TXDATANUM, &bRegister));

                /* Disable DataEn */
                bRegister &= (uint8_t)~(uint8_t)PHHAL_HW_NCx3320_BIT_DATAEN;

                /* Write changed register */
                PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_TXDATANUM, bRegister));
            }
            else
            {
                /* Reset NoData flag */
            ...
            if(pDataParams->bJewelActivated == PH_ON)
            {
                ...
                PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_TransmitJewelCmd(pDataParams, wTmpBufferLen, pTmpBuffer));
                ...
                PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_TCONTROL, &bRegister));
                ...
                PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_TCONTROL, bRegister));
            }

            /* Terminate a possibly running command */
            PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_COMMAND, PHHAL_HW_NCx3320_CMD_IDLE));

            /* Flush FiFo */
            PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_FlushFifo(pDataParams));

            /* clear all IRQ0 flags */
            PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(
                pDataParams,
            ...
            {
                bIrq0WaitFor = PHHAL_HW_NCx3320_BIT_LOALERTIRQ;
            }
    ...
    if (pDataParams->bPollGuardTimeFlag == PH_ON)
    {
        ...
        /* retrieve content of Timer Control register */
        PH_CHECK_SUCCESS_FCT(status, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_TCONTROL, &bRegister));

        /* Check if timer is running for guard time to expire, else don't wait for timer IRQ. */
        if ((0U != ((bRegister & PHHAL_HW_NCx3320_BIT_T1RUNNING))) &&
            ((statusTmp == PH_ERR_SUCCESS) ||
            (statusTmp == PH_ERR_BUFFER_OVERFLOW)))
        {
            PH_CHECK_SUCCESS_FCT(status, phhalHw_Ncx3320_WaitIrq(pDataParams, PH_ON, PH_OFF, 0x00, PHHAL_HW_NCx3320_BIT_TIMER1IRQ, NULL, &bIrq1Reg));
        }

        /* Stop Timer T0 and T1 used for Poll guard time. */
        bRegister &= (uint8_t)~(uint8_t)(PHHAL_HW_NCx3320_BIT_T0RUNNING | PHHAL_HW_NCx3320_BIT_T1RUNNING);
        PH_CHECK_SUCCESS_FCT(status, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_TCONTROL, bRegister));
        /* Clear Timer Interrupt */
        PH_CHECK_SUCCESS_FCT(status, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ1, (uint8_t)~(uint8_t)PHHAL_HW_NCx3320_BIT_SET));

        /* Apply FWT/FDT delay from dataparams. */
        if (pDataParams->bTimeoutUnit == PHHAL_HW_TIME_MICROSECONDS)
        {
            PH_CHECK_SUCCESS_FCT(status, phhalHw_Ncx3320_SetFdt(
                pDataParams,
    ...
    /* start the command */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_COMMAND, PHHAL_HW_NCx3320_CMD_TRANSCEIVE));
    ...
    if ((0U == (bIrq0Reg & PHHAL_HW_NCx3320_BIT_IDLEIRQ))
        && (0U == (bIrq0Reg & PHHAL_HW_NCx3320_BIT_ERRIRQ)))
    {
        /* Clear high-alert irq flag */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ0, PHHAL_HW_NCx3320_BIT_HIALERTIRQ));

        /* Change wait IRQs */
        if(0U != (pDataParams->bRxMultiple))
        {
            bIrq0WaitFor = PHHAL_HW_NCx3320_BIT_RXIRQ |PHHAL_HW_NCx3320_BIT_IDLEIRQ | PHHAL_HW_NCx3320_BIT_EMDIRQ;
            bIrq1WaitFor = PHHAL_HW_NCx3320_BIT_TIMER1IRQ;
        }
        else
        {
        ...
        else
        {
            ...
            status = phhalHw_Ncx3320_CheckForEmdError(pDataParams, bIrq0WaitFor, bIrq1WaitFor, &dwMultiReg, &dwSaveReg);
    ...
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_SetConfig(pDataParams, PHHAL_HW_CONFIG_TXLASTBITS, 0x00U));
    }
    ...
    {
        /* Read out TxDataNum register */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_TXDATANUM, &bRegister));

        /* Enable DataEn */
        bRegister |= PHHAL_HW_NCx3320_BIT_DATAEN;

        /* Write changed register */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_TXDATANUM, bRegister));
    }

    /* Timeout handling */
    if ((0U != (bIrq1Reg & PHHAL_HW_NCx3320_BIT_TIMER1IRQ)) &&
        (0U == (bIrq0Reg & PHHAL_HW_NCx3320_BIT_EMDIRQ)))
    {
        /* retrieve content of Timer Control register */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_TCONTROL, &bRegister));
        ...
        if(0U != (wVerifyRxCrc5))
        {
            ...
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ0, &bRegister));
            if(0U != (bRegister & PHHAL_HW_NCx3320_BIT_ERRIRQ))
            {
                 PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_ERROR, &bRegister));
                 /* If Either of MINFRAME\COLLDET\PROT\INTEG error is set treat that as collision */
                if(
                    (0U != (bRegister & PHHAL_HW_NCx3320_BIT_MINFRAMEERR)) ||
                    (0U != (bRegister & PHHAL_HW_NCx3320_BIT_COLLDET)) ||
                    (0U != (bRegister & PHHAL_HW_NCx3320_BIT_PROTERR)) ||
                    (0U != (bRegister & PHHAL_HW_NCx3320_BIT_INTEGERR)))
                 {
    ...
    if (status == PH_ERR_SUCCESS)
    {
        ...
        do
        {
            ...
            if (0U != (bIrq0Reg & PHHAL_HW_NCx3320_BIT_RXIRQ))
            {
                break;
            }
            else
            {
                ...
                PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(
                    pDataParams,
                    PHHAL_HW_NCx3320_REG_IRQ0,
                    (uint8_t)PHHAL_HW_NCx3320_BIT_HIALERTIRQ));
            }

            bIrq0WaitFor_tmp &= (uint8_t)~(uint8_t)(PHHAL_HW_NCx3320_BIT_HIALERTIRQ
                                    |  PHHAL_HW_NCx3320_BIT_RXIRQ | PHHAL_HW_NCx3320_BIT_IDLEIRQ);

            /* Enabling IRQs, so that already occurred events are not missed. */
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ0EN, &bRegister));

            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ0EN,
                bIrq0WaitFor | (bRegister & PHHAL_HW_NCx3320_BIT_IRQINV)));

            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ1EN,
                bIrq1WaitFor | PHHAL_HW_NCx3320_BIT_IRQPUSHPULL | PHHAL_HW_NCx3320_BIT_GLOBALIRQ));
        ...
        */
        status =  phhalHw_Ncx3320_GetErrorStatus(pDataParams, &wTmpBufferLen, &wTmpBufferSize);
    ...
    if (status == PH_ERR_SUCCESS)
    {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXBITCTRL, &bRegister));
    ...
    if((0U != (pDataParams->bRxMultiple)) &&
        (status == PH_ERR_SUCCESS))
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_FrameRxMultiplePkt(pDataParams));
    }
...
phStatus_t phhalHw_Ncx3320_Transmit(
                                  phhalHw_Ncx3320_DataParams_t * pDataParams,
                                  uint16_t wOption,
                                  uint8_t * pTxBuffer,
                                  uint16_t wTxLength
                                  )
{
    ...
    if (0U != (bIrq0Reg & PHHAL_HW_NCx3320_BIT_ERRIRQ))
    {
        /* read the error register */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_ERROR, &bError));

        /* no data error */
        if (0U != (bError & PHHAL_HW_NCx3320_BIT_NODATAERR))
        {
            status = PH_ERR_INTERNAL_ERROR;
        }
        /* buffer overflow */
        else if (0U != (bError & PHHAL_HW_NCx3320_BIT_FIFOOVL))
        {
...
phStatus_t phhalHw_Ncx3320_Receive(
                                 phhalHw_Ncx3320_DataParams_t * pDataParams,
                                 uint16_t wOption,
                                 uint8_t ** ppRxBuffer,
                                 uint16_t * pRxLength
                                 )
{
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXWAIT, &bRegRxWait));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXWAIT, 0U));
    ...
    /* start the command */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_COMMAND, PHHAL_HW_NCx3320_CMD_RECEIVE));
    ...
    if (status == PH_ERR_SUCCESS)
    {
        ...
        do
        {
            /* read interrupt status */
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ0, &bIrq0Reg));
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ1, &bIrq1Reg));
            ...
            if (0U != (bIrq0Reg & PHHAL_HW_NCx3320_BIT_HIALERTIRQ))
            {
                              PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(
                                            pDataParams,
                                            PHHAL_HW_NCx3320_REG_IRQ0,
        ...
        else if (0U != (bError & PHHAL_HW_NCx3320_BIT_COLLDET))
        {
            ...
            else
            {
                ...
                PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXCOLL, &bRegister));

                /* check if collpos is valid */
                if (0U != (bRegister & PHHAL_HW_NCx3320_BIT_COLLPOSVALID))
                {
                    /* retrieve collpos */
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXWAIT, bRegRxWait));
    ...
    )
{
    ...
    case PHHAL_HW_CARDTYPE_ISO14443A:
        if(pDataParams->bOpeMode != RD_LIB_MODE_ISO)
        {
            ...
            SET_NCx3320_SHADOW(wNcx3320_Emvco_I14443a_DefaultShadow);

            /* Max EMD noise length is 2 bytes (excluding 2 bytes of CRC) */
            pDataParams->bEmdNoiseMaxDataLen = PHHAL_HW_NCx3320_EMV_NOISE_MAXLEN;
        }
        else
        {
            /* Use 14443a default shadow */
            SET_NCx3320_SHADOW(wNcx3320_DefaultShadow_I14443a);
    ...
    case PHHAL_HW_CARDTYPE_ISO14443B:
        ...
        if(pDataParams->bOpeMode != RD_LIB_MODE_ISO)
        {
            if (pDataParams->bOpeMode == RD_LIB_MODE_EMVCO)
            {
                ...
                SET_NCx3320_SHADOW(wNcx3320_Emvco_I14443b_DefaultShadow);
            }
            else
            {
                /* Use 14443b NFC Forum shadow */
                SET_NCx3320_SHADOW(wNcx3320_NFC_I14443b_DefaultShadow);
            }
            ...
            * is set to 3 bytes (2u + 1 bytes). */
            pDataParams->bEmdNoiseMaxDataLen = PHHAL_HW_NCx3320_EMV_NOISE_MAXLEN + 1U;
        }
        else
        {
            /* Use 14443b shadow */
            SET_NCx3320_SHADOW(wNcx3320_DefaultShadow_I14443b);
            pDataParams->bEmdNoiseMaxDataLen = 0x00;
        }
        break;
    ...
    if (bMode != PHHAL_HW_CARDTYPE_CURRENT)
    {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T2RELOADHI, 0x03U));
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T2RELOADLO, 0xFFU));
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXBITCTRL, PHHAL_HW_NCx3320_BIT_VALUESAFTERCOLL));

        /* Init RxSOFD register */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXSOFD, 0x00U));
    ...
    if (0U != (bUseDefaultShadow))
    {
        ...
        (void)memset(pDataParams->wCfgShadow, 0x00, PHHAL_HW_NCx3320_SHADOW_COUNT);
...
phStatus_t phhalHw_Ncx3320_SetConfigMultiple(
                                    phhalHw_Ncx3320_DataParams_t * pDataParams,
                                   ...
                                   )
{
    ...
    case PHHAL_HW_CONFIG_RXALIGN:
        ...
        /* check parameter */
        if (wValue > (PHHAL_HW_NCx3320_MASK_RXALIGN >> 4U))
        {
            return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
        }
        ...
        if (bValue != bRegister)
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXBITCTRL, bValue));
        }
    ...
    case PHHAL_HW_CONFIG_TXWAIT_MS:
        ...
        /* Set TxWait */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_Wait(pDataParams, PHHAL_HW_TIME_MILLISECONDS, wValue));
        break;
    ...
    case PHHAL_HW_CONFIG_TXDATARATE_FRAMING:
        ...
        switch (pDataParams->bCardType)
        {
            ...
            case PHHAL_HW_CARDTYPE_FELICA_212:
                ...
                {
                    /* Use ISO14443 shadow */
                    SET_NCx3320_SHADOW(wNcx3320_DefaultShadow_NfcDep);
        ...
        if(wShadowCount > 0U)
        {
            for (wIndex = 0; wIndex < wShadowCount; ++wIndex)
            {
                ...
                {
                    /* Use internal SetConfig */
                    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_SetConfig_Int(pDataParams, wConfigShadow, pShadowDefault[(wIndex << 1U) + 1U]));
                }
            }
        }
    ...
    case PHHAL_HW_CONFIG_TIMEOUT_VALUE_MS:
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ConfigTimeout(pDataParams, wConfig, wValue));
        break;
    ...
    case PHHAL_HW_CONFIG_SYMBOL_START:
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_FRAMECON, &bRegister));
        ...
        if (bValue != bRegister)
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_FRAMECON, bValue));
        }
        break;
    ...
    case PHHAL_HW_CONFIG_CRCTYPE:
        ...
        {
            bValue = PHHAL_HW_NCx3320_MASK_CRCTYPE16;
        }

        /* in case of 18000P3M3 for CRC5 calculation we need preset value of 9 which is not supported by NCx3320
         * So flag is enable to perform software calculation of CRC5 in NCx3320 HAL
         */
        ...
        {
            bValue |= PHHAL_HW_NCx3320_BIT_CRCINVERT;
        }
        /* Set TXPresetVal I18000p3m3  for CRC16 preset 0xFFFFU is used */
        if ((pDataParams->bCardType == PHHAL_HW_CARDTYPE_I18000P3M3) &&
            (0U != (wValue & PHHAL_HW_CRCTYPE_CRC16)))
        {
            bValue |= PHHAL_HW_NCx3320_MASK_CRCPRESETVAL;
        }
        if(pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXCRC] == PH_ON)
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_TXCRCCON, &bRegister));
            /* update CRC value */
            bValue |= bRegister;
            /* Only perform the operation, if the new value is different */
            if (bValue != bRegister)
            {
                PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_TXCRCCON, bValue));
            }
        }
        if(pDataParams->wCfgShadow[PHHAL_HW_CONFIG_RXCRC] == PH_ON)
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXCRCCON, &bRegister));
            /* update CRC value */
            bValue |= bRegister;
            ...
            if(0U != (wValue & PHHAL_HW_CRCTYPE_CRC16))
            {
                bValue &= (uint8_t)~(uint8_t)PHHAL_HW_NCx3320_BIT_RXFORCECRCWRITE;
            }

            /* Only perform the operation, if the new value is different */
            if (bValue != bRegister)
            {
                PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXCRCCON, bValue));
            }
        }
        break;
    ...
    case PHHAL_HW_CONFIG_DISABLE_MF_CRYPTO1:
        ...
        if (wValue != PH_OFF)
        {
            ...
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_STATUS, &bRegister));
            ...
            if (bValue != bRegister)
            {
                PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_STATUS, bValue));
            }
        }
        break;
    ...
    case PHHAL_HW_CONFIG_MAX_PRECACHED_BYTES:
        ...
        if (((pDataParams->bFifoSize == PH_NXPNFCRDLIB_CONFIG_NCx3320_VALUE_FIFOSIZE_255) &&
            (wValue > 0xFFU)) ||
            ((pDataParams->bFifoSize == PH_NXPNFCRDLIB_CONFIG_NCx3320_VALUE_FIFOSIZE_512) &&
                (wValue > 0x200U)))
        {
            return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
        }
    ...
    case PHHAL_HW_CONFIG_SERIAL_BITRATE:
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_SERIALSPEED, bRegister));
        break;
    ...
    case PHHAL_HW_NCx3320_CONFIG_FIFOSIZE:
        ...
        if (wValue == PH_NXPNFCRDLIB_CONFIG_NCx3320_VALUE_FIFOSIZE_255)
        {
            ...
            pDataParams->wMaxPrecachedBytes = PH_NXPNFCRDLIB_CONFIG_NCx3320_PRECACHED_255BYTES;

        }
        else if (wValue == PH_NXPNFCRDLIB_CONFIG_NCx3320_VALUE_FIFOSIZE_512)
        {
            ...
            /* Also reset max. precached bytes. */
            pDataParams->wMaxPrecachedBytes = PH_NXPNFCRDLIB_CONFIG_NCx3320_PRECACHED_512BYTES;
    ...
    case PHHAL_HW_CONFIG_RXMULTIPLE:
            ...
            if (wValue == PH_ON)
            {
                PH_CHECK_SUCCESS_FCT(statusTmp,
                    phhalHw_Ncx3320_ReadRegister(
                    pDataParams,
                    PHHAL_HW_NCx3320_REG_RXCTRL,
                    &bRegister));
                bRegister |= PHHAL_HW_NCx3320_BIT_RXMULTIPLE;

                PH_CHECK_SUCCESS_FCT(statusTmp,
                    phhalHw_Ncx3320_WriteRegister(
                    pDataParams,
                    PHHAL_HW_NCx3320_REG_RXCTRL,
                    bRegister
                    ));
            ...
            else
            {
                ...
                bRegister = bRegister & (uint8_t)(~(uint8_t)PHHAL_HW_NCx3320_BIT_RXMULTIPLE);
                PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(
                    pDataParams,
                    PHHAL_HW_NCx3320_REG_RXCTRL,
                    bRegister
	...
	case PHHAL_HW_CONFIG_EMD_MODE:
		if ((wValue != PH_ON) && (wValue != PH_OFF))
		{
			return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
		}
		pDataParams->bEmdMode = (uint8_t) wValue;
		break;

	case PHHAL_HW_CONFIG_EMD_TIMEOUT:
		if (wValue == 0U || wValue > 255U)
		{
			return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
		}
		pDataParams->bEmdTimeout = (uint8_t)wValue;
		break;

	case PHHAL_HW_CONFIG_EMD_RXTHRESHOLD:
		pDataParams->bEmdThreshold = (uint8_t) wValue;
		break;

	case PHHAL_HW_CONFIG_EMD_RECEIVEGAIN:
		pDataParams->bEmdReceiveGain = (uint8_t) wValue;
		break;
    ...
    case PHHAL_HW_CONFIG_POLL_GUARD_TIME_US:
        ...
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_T0CONTROL, &bRegister));
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T0CONTROL,
                bRegister | PHHAL_HW_NCx3320_BIT_TAUTORESTARTED | PHHAL_HW_NCx3320_BIT_TSTOP_RX | PHHAL_HW_NCx3320_BIT_TSTART_TX | PHHAL_HW_NCx3320_VALUE_TCLK_1356_MHZ));

            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_T1CONTROL, &bRegister));
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T1CONTROL,
                bRegister | PHHAL_HW_NCx3320_BIT_TSTOP_RX | PHHAL_HW_NCx3320_BIT_TSTART_TX | PHHAL_HW_NCx3320_VALUE_TCLK_T0));
        }

        /* Configure and start timer. */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_Wait(pDataParams, PHHAL_HW_TIME_MICROSECONDS, wValue));
        break;

    case PHHAL_HW_CONFIG_RFCA:
        break;

    case PHHAL_HW_NCx3320_CONFIG_WATERLEVEL:
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(
            pDataParams,
            PHHAL_HW_NCx3320_REG_WATERLEVEL,
            (uint8_t)wValue));
    ...
    case PHHAL_HW_NCx3320_CONFIG_LPCD_CHARGEPUMP:
        if (wValue == PH_ON)
        {
            pDataParams->bLpcdOption |= PHHAL_HW_NCx3320_BIT_LPCD_CHARGEPUMP;
        }
        else if (wValue == PH_OFF)
        {
            pDataParams->bLpcdOption &= (uint8_t)~(uint8_t)PHHAL_HW_NCx3320_BIT_LPCD_CHARGEPUMP;
        }
        else
        {
            return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
    ...
    case PHHAL_HW_NCx3320_CONFIG_LPCD_FILTER:

        if ((wValue & PHHAL_HW_NCx3320_FILTER_MASK) == PH_ON)
        {
            pDataParams->bLpcdOption |= PHHAL_HW_NCx3320_BIT_LPCD_FILTER;

            /* Higher nibble of bLpcdOption is used to store Option which is used to determine the configuration of I and Q values. */
            if ((wValue & PHHAL_HW_NCx3320_FILTER_OPTION_MASK) == PHHAL_HW_NCx3320_FILTER_OPTION1)
            {
                pDataParams->bLpcdOption |= PHHAL_HW_NCx3320_FILTER_BIT_OPTION1;
                pDataParams->bLpcdOption &= (uint8_t)~(uint8_t)PHHAL_HW_NCx3320_FILTER_BIT_OPTION2;
            }
            else if ((wValue & PHHAL_HW_NCx3320_FILTER_OPTION_MASK) == PHHAL_HW_NCx3320_FILTER_OPTION2)
            {
                pDataParams->bLpcdOption |= PHHAL_HW_NCx3320_FILTER_BIT_OPTION2;
                pDataParams->bLpcdOption &= (uint8_t)~(uint8_t)PHHAL_HW_NCx3320_FILTER_BIT_OPTION1;
            }
            else
            {
                return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
        ...
        else if ((wValue & PHHAL_HW_NCx3320_FILTER_MASK) == PH_OFF)
        {
            pDataParams->bLpcdOption &= (uint8_t)~(uint8_t)(PHHAL_HW_NCx3320_BIT_LPCD_FILTER |
                PHHAL_HW_NCx3320_FILTER_BIT_OPTION1 | PHHAL_HW_NCx3320_FILTER_BIT_OPTION2);
        }
        else
        {
            return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
		...
		case PHHAL_HW_NCx3320_CONFIG_LPCD_CWAMP:

			if (wValue >= (uint16_t) 0x04U)
			{
				return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
			}
			...
			PH_CHECK_SUCCESS_FCT(statusTmp,
					phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_TXAMP, &bRegister));

			/* Configure transmitter amplitude */
			bValue = (bRegister & (uint8_t) ~(uint8_t) PHHAL_HW_NCx3320_MASK_CW_AMPLITUDE)
					| (uint8_t) (wValue << 6U);
			...
			{
				PH_CHECK_SUCCESS_FCT(statusTmp,
						phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_TXAMP, bValue));
			}
			break;

		case PHHAL_HW_NCx3320_CONFIG_LPCD_CWMAX:

			/* Read out TxCon register */
			PH_CHECK_SUCCESS_FCT(statusTmp,
					phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_DRVCON, &bRegister));

			if (wValue == PH_OFF)
			{
				/* Clear the Cwmax bit */
				bValue = bRegister & (uint8_t) ~(uint8_t) PHHAL_HW_NCx3320_BIT_CWMAX;
			}
			else
			{
				/* Set the Cwmax bit  */
				bValue = bRegister | (uint8_t) PHHAL_HW_NCx3320_BIT_CWMAX;
			}
			...
			{
				PH_CHECK_SUCCESS_FCT(statusTmp,
						phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_DRVCON, bValue));
			}
			break;

#ifdef NXPBUILD__PHHAL_HW_NCx3320_CUSTOM_DEBUG
    case PHHAL_HW_NCx3320_CONFIG_TX_AMPLITUDE:

        /* Parameter masking */
        wValue &= PHHAL_HW_NCx3320_MASK_CW_AMPLITUDE;

        /* Read out TxAmp register */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_TXAMP, &bRegister));

        /* Set Tx Amplitude value */
        bValue = (bRegister & (uint8_t)~(uint8_t)PHHAL_HW_NCx3320_MASK_CW_AMPLITUDE) | (uint8_t)wValue;

        /* Only perform the operation, if the new value is different */
        if (bValue != bRegister)
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_TXAMP, bValue));
        }
        break;
#endif /* NXPBUILD__PHHAL_HW_NCx3320_CUSTOM_DEBUG */
                                   ...
                                   )
{
    ...
    case PHHAL_HW_CONFIG_DISABLE_MF_CRYPTO1:
        ...
        /* Check Crypto1On bit */
        if (0U != (bRegister & PHHAL_HW_NCx3320_BIT_CRYPTO1ON))
        {
    ...
    case PHHAL_HW_CONFIG_SERIAL_BITRATE:
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_SERIALSPEED, &bRegister));
    ...
    case PHHAL_HW_CONFIG_ACTIVEMODE:
        ...
        /* Return PH_OFF, as this is not applicable in-case of Ncx3320. */
        *pValue = PH_OFF;
        break;
	...
	case PHHAL_HW_CONFIG_EMD_MODE:
		*pValue = (uint16_t) pDataParams->bEmdMode;
		break;

	case PHHAL_HW_CONFIG_EMD_TIMEOUT:
		*pValue = (uint16_t) pDataParams->bEmdTimeout;
		break;

	case PHHAL_HW_CONFIG_EMD_RXTHRESHOLD:
		*pValue = (uint16_t) pDataParams->bEmdThreshold;
		break;

	case PHHAL_HW_CONFIG_EMD_RECEIVEGAIN:
		*pValue = (uint16_t) pDataParams->bEmdReceiveGain;
		break;
    ...
    case PHHAL_HW_CONFIG_INT_RF_ON:
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(
            ...
            &bRegister));
        if(0U != (bRegister & PHHAL_HW_NCx3320_BIT_TXEN))
        {
            *pValue = PH_ON;
        }
        else
                                 ...
                                 )
{
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_DRVMODE, &bRegister));

    /* Enable Drivers */
    bRegister |= PHHAL_HW_NCx3320_BIT_TXEN;

    /* Write DrvMode register */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_DRVMODE, bRegister));
                              ...
                              )
{
    ...
    if ((pDataParams->bCardType == PHHAL_HW_CARDTYPE_UNKNOWN) &&
        (pDataParams->bPollGuardTimeFlag == PH_OFF))
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_T0CONTROL, &bT0Ctrl));
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T0CONTROL, bT0Ctrl | PHHAL_HW_NCx3320_BIT_TAUTORESTARTED | PHHAL_HW_NCx3320_VALUE_TCLK_1356_MHZ));

        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_T1CONTROL, &bT1Ctrl));
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T1CONTROL, bT1Ctrl | PHHAL_HW_NCx3320_VALUE_TCLK_T0));
    }
    ...
    /* retrieve content of Timer Control register */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_TCONTROL, &bControlReg));

    /* clear all irq flags */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ1, (uint8_t)~(uint8_t)PHHAL_HW_NCx3320_BIT_SET));
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_TCONTROL, bControlReg));
    ...
    )
{
    ...
    {
        /* check key number */
        if (wKeyNo >= (PHHAL_HW_NCx3320_MAX_NUM_KEYS >> 1U))
        {
            return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
        }
    ...
    {
        /* check key number */
        if (wKeyNo >= ((PHHAL_HW_NCx3320_MAX_NUM_KEYS >> 1U) - 1U))
        {
            return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
        }
    ...
    )
{
    ...
    if (wValue == PH_ON)
    {
        ...
        PH_CHECK_SUCCESS_FCT(status, phhalHw_Ncx3320_ConfigTimeout(pDataParams,
...
phStatus_t phhalHw_Ncx3320_AsyncAbort(
                                    phhalHw_Ncx3320_DataParams_t * pDataParams
                                    )
{
#ifndef _WIN32
...
phStatus_t phhalHw_Ncx3320_I18000p3m3Inventory(
                                             phhalHw_Ncx3320_DataParams_t  * pDataParams,
                                             uint8_t * pSelCmd,
                                             uint8_t bSelCmdLen,
    ...
    *pRxBufferLen = 0;
    ...
    if(bTSprocessing != PHHAL_HW_I18000P3M3_GET_MAX_RESPS)
    {
        ...
        if((bSelCmdLen > 0U) &&
            (pSelCmd != NULL))
        {
            status = phhalHw_Ncx3320_18000p3m3_Sw_Select(
                    pDataParams,
                    pSelCmd,
                    bSelCmdLen,
    ...
    /* Perform BEGIN ROUND */
    status = phhalHw_Ncx3320_18000p3m3_Sw_BeginRound(pDataParams, pBeginRndCmd);
    ...
    if(bTSprocessing == PHHAL_HW_I18000P3M3_ONE_TS_ONLY)
    {
        ...
        {
            /* Perform ACK */
            status = phhalHw_Ncx3320_18000p3m3_Sw_Ack(
                pDataParams,
                &pRxBuffer,
                &wRxBufferLen,
                &bRxLastBits);
    ...
    else if(bTSprocessing == PHHAL_HW_I18000P3M3_GET_MAX_RESPS)
    {
        ...
        while(0U != (wNumSlotsDec--))
        {
            ...
            else if((status & PH_ERR_MASK) == PH_ERR_SUCCESS)
            {
                ...
                if((status & PH_ERR_MASK) == PH_ERR_SUCCESS)
                {
                    ...
                    if (wNumSlotsDec == 0U)
                    {
                        ...
                        (void)phhalHw_Ncx3320_18000p3m3_Sw_NextSlot(pDataParams);
                    }
                }
                else if (((status & PH_ERR_MASK) == PH_ERR_COLLISION_ERROR) ||
                         ((status & PH_ERR_MASK) == PH_ERR_INTEGRITY_ERROR))
                {
                    ...
                    (void)phhalHw_Ncx3320_Wait(pDataParams, PHHAL_HW_TIME_MICROSECONDS, 1210);
                }
                else if((status & PH_ERR_MASK) == PH_ERR_IO_TIMEOUT)
                {
                    /* prefix 3 control bytes for each response */
            ...
            {
                /* Send CloseSlot command */
                status = phhalHw_Ncx3320_18000p3m3_Sw_NextSlot(pDataParams);
            }
        }
    ...
    else if(bTSprocessing == PHHAL_HW_I18000P3M3_GET_TAG_HANDLE)
    {
        ...
        if ((status & PH_ERR_MASK) == PH_ERR_SUCCESS)
        {
            ...
            if((status & PH_ERR_MASK) == PH_ERR_SUCCESS)
            {
                ...
                status = phhalHw_Ncx3320_18000p3m3_Sw_ReqRn(
                    pDataParams,
                    &pRxBuffer);
...
phStatus_t phhalHw_Ncx3320_I18000p3m3ResumeInventory(
                                                   phhalHw_Ncx3320_DataParams_t * pDataParams,
                                                   uint8_t ** ppRxBuffer,
                                                   uint16_t * pRxBufferLen
                                                   )
...
phStatus_t phhalHw_Ncx3320_EventWait(phhalHw_Ncx3320_DataParams_t * pDataParams, uint32_t dwEventTimeout)
{
...
phStatus_t phhalHw_Ncx3320_EventConsume(phhalHw_Ncx3320_DataParams_t * pDataParams)
{
...
phStatus_t phhalHw_Ncx3320_DeInit(
                                phhalHw_Ncx3320_DataParams_t * pDataParams
                                )
{
    phStatus_t PH_MEMLOC_REM status;
    ...
    PH_CHECK_SUCCESS_FCT(status, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ0EN, &bRegister));
    /* Clear IRQ0 interrupt sources */
    bRegister = 0x00;
    PH_CHECK_SUCCESS_FCT(status, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ0EN, bRegister));

    PH_CHECK_SUCCESS_FCT(status, phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ1EN, &bRegister));
    /* Clear IRQ1 interrupt sources */
    bRegister = 0x00;
    PH_CHECK_SUCCESS_FCT(status, phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ1EN, bRegister));
...
static void phhalHw_Ncx3320_Reset(void)
{
    ...
    /* delay of ~2 ms */
    (void)phDriver_TimerStart(PH_DRIVER_TIMER_MILLI_SECS, PHHAL_HW_NCx3320_RESET_DELAY_MILLI_SECS, NULL);
                                                 ...
                                                 )
{
    ...
    for(wIndex = 0U; wIndex < (wJewelBufferLen - 1U); wIndex++)
    {
        ...
        /* Perform actual write to FIFO */
        PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_WriteFifo(
            pDataParams,
            pTmpBuffer++,
            1U,
            &wFifoBytes));
        ...
        PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(
            pDataParams,
            PHHAL_HW_NCx3320_REG_COMMAND,
            PHHAL_HW_NCx3320_CMD_TRANSMIT));

        do
        {
#ifndef NXPBUILD__PH_PLATFORM_DELEGATE
            /* wait until the transmission is finished */
            PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_WaitIrq(
                pDataParams,
                PH_ON,
                PH_OFF,
                bIrq0WaitFor,
        ...
        if (pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXLASTBITS] != 0U)
        {
            PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Ncx3320_SetConfig(pDataParams, PHHAL_HW_CONFIG_TXLASTBITS, 0x00U));
        }
    }
...
phStatus_t phhalHw_Ncx3320_DetectEMD(phhalHw_Ncx3320_DataParams_t * pDataParams)
{
	phStatus_t status    = PH_ERR_SUCCESS;
	phStatus_t statusTmp = PH_ERR_SUCCESS;
	uint8_t bRegister    = 0u;
	uint8_t bValue       = 0u;
	uint8_t *pRcvBuffer;
	uint16_t wRcvBytes;
	uint16_t wConfig     = 0u;
	static uint8_t bTimer0      = 0u;
	static uint8_t bTimer1      = 0u;

	/* get EMD timeout value */
	status = phhalHw_Ncx3320_GetConfig(pDataParams, PHHAL_HW_CONFIG_EMD_TIMEOUT, &wConfig);
	if (status != PH_ERR_SUCCESS)
	{
		wConfig = 10U;
	}
	/* set timeout value */
	phhalHw_Ncx3320_SetConfig(pDataParams, PHHAL_HW_CONFIG_TIMEOUT_VALUE_MS, wConfig);
	/* retrieve content of Timer 0 Control register */
	phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_T0CONTROL, &bTimer0);
	/* manually start T0 */
	bRegister = bTimer0 & ((uint8_t) ~((uint8_t) PHHAL_HW_NCx3320_MASK_TSTART));
	phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T0CONTROL, bRegister);
	/* retrieve content of Timer 1 Control register */
	phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_T1CONTROL, &bTimer1);
	/* manually start T1 */
	bRegister = bTimer1 & ((uint8_t) ~((uint8_t) PHHAL_HW_NCx3320_MASK_TSTART));
	phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T1CONTROL, bRegister);

	/* retrieve content of Timer Control register */
	phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_TCONTROL, &bRegister);
	/* manually start T0 */
	...
	phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_TCONTROL, bRegister);

	statusTmp = phhalHw_Ncx3320_Receive(pDataParams, PH_RECEIVE_DEFAULT,
			&pRcvBuffer, &wRcvBytes);

	/* retrieve content of Timer Control register */
	phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_TCONTROL, &bRegister);
	/* manually start T0 */
	bRegister &= (uint8_t) ~((uint8_t) PHHAL_HW_NCx3320_BIT_T0RUNNING);
	bRegister |= PHHAL_HW_NCx3320_BIT_T0STARTSTOPNOW;
	/* manually start T1 */
	bRegister &= (uint8_t) ~((uint8_t) PHHAL_HW_NCx3320_BIT_T1RUNNING);
	bRegister |= PHHAL_HW_NCx3320_BIT_T1STARTSTOPNOW;
	phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_TCONTROL, bRegister);

	/* restore timer 0 and 1 settings for reception */
	/* retrieve content of Timer 0 Control register */
	phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T0CONTROL, bTimer0);
	phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T1CONTROL, bTimer1);

	/* Anything else than a timeout means there is a disturber */
	if ((statusTmp & PH_ERR_MASK) != PH_ERR_IO_TIMEOUT)
	{
		/* Receiver gain setting*/
		phhalHw_Ncx3320_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXANA, &bRegister);
		bValue = bRegister & ((uint8_t) ~(uint8_t) PHHAL_HW_NCx3320_MASK_RCV_GAIN);
		/* Get Receive Gain configuration */
		(void)phhalHw_Ncx3320_GetConfig(pDataParams, PHHAL_HW_CONFIG_EMD_RECEIVEGAIN, &wConfig);
		bValue |= (uint8_t)wConfig&(uint8_t)PHHAL_HW_NCx3320_MASK_RCV_GAIN;

		/* Only perform the operation, if the new value is different */
		if (bValue != bRegister)
		{
			phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXANA, bValue);
		}
		/* Get Rx Threshold configuration */
		(void)phhalHw_Ncx3320_GetConfig(pDataParams, PHHAL_HW_CONFIG_EMD_RXTHRESHOLD, &wConfig);
		/* Threshold setting*/
		phhalHw_Ncx3320_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXTHRESHOLD, (uint8_t)wConfig);
	}
