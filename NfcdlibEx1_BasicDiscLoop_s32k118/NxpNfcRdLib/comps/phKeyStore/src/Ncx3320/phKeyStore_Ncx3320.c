    ...
    )
{
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_ReadRegister(pDataParams->pHalDataParams, PH_KEYSTORE_NCx3320_REG_IRQ0, &bRegValue));

    if ( (0U == ((PHHAL_HW_NCx3320_BIT_IDLEIRQ & bRegValue))))
    {
        /* stop the command */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteRegister(
            pDataParams->pHalDataParams,
